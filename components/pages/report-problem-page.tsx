...
export default function ReportProblemPage({ onSubmit, onBack }: ReportProblemPageProps) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-blue-50 to-background">
      {/* Header */}
      <div className="bg-secondary text-white p-8 flex items-center justify-between shadow-xl">
        <button onClick={onBack} className="text-4xl font-bold hover:opacity-80 transition-all min-w-[120px] text-left">
          ← กลับ
        </button>
        <h1 className="text-4xl md:text-5xl font-bold">แจ้งปัญหาในชุมชน</h1>
        <div className="w-[120px]" />
      </div>

      {/* Main Content */}
      <div className="flex-1 flex flex-col p-8 md:p-12 overflow-y-auto max-w-3xl mx-auto w-full">
        {/* Problem Type Selection */}
        <div className="mb-10">
          <p className="text-4xl font-bold mb-6 text-foreground">ปัญหาที่พบคืออะไร?</p>
          <div className="grid grid-cols-2 gap-5 md:grid-cols-3">
            {problemTypes.map((problem) => (
              <button
                key={problem.id}
                onClick={() => setSelectedProblem(problem.id)}
                className={`rounded-2xl p-8 font-bold text-xl transition-all border-4 min-h-[160px] ${
              ...
              >
                <div className="flex flex-col items-center gap-4">
                  ...
                  <span className="text-xl text-center leading-tight">{problem.name}</span>
                </div>
              </button>
            ))}
          </div>
        ...
        {/* Location Selection */}
        <div className="mb-10">
          <p className="text-4xl font-bold mb-6 text-foreground">สถานที่เกิดเหตุ</p>
          <div className="space-y-4">
            {locations.map((location) => (
              <button
                key={location}
                onClick={() => setSelectedLocation(location)}
                className={`w-full rounded-2xl p-8 font-bold text-2xl transition-all border-4 text-left ${
        ...
        <div className="mb-10">
          <p className="text-4xl font-bold mb-6 text-foreground">รายละเอียดเพิ่มเติม (ถ้ามี)</p>
          <textarea
            value={description}
            onChange={(e) => setDescription(e.target.value)}
            placeholder="อธิบายปัญหาเพิ่มเติม..."
            className="w-full rounded-2xl p-6 border-4 border-border focus:border-secondary focus:outline-none text-2xl resize-none"
            rows={4}
          />
        </div>
        ...
        <button
          ...
          className={`w-full rounded-3xl p-10 font-bold text-4xl shadow-2xl transition-all min-h-[120px] ${
            isFormValid
