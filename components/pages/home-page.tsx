...
export default function HomePage({ onEmergency, onReport, onNews }: HomePageProps) {
  return (
    <div className="flex flex-col min-h-screen">
      ...
      {/* Main Content */}
      <div className="flex-1 flex flex-col items-center justify-center p-4 sm:p-8 gap-5 sm:gap-10 py-6 sm:py-12">
        {/* Action Buttons */}
        <div className="w-full max-w-3xl space-y-4 sm:space-y-8">
          {/* Emergency Button */}
          <button
            onClick={onEmergency}
            className="w-full bg-destructive hover:bg-destructive/90 text-destructive-foreground rounded-2xl sm:rounded-3xl p-6 sm:p-12 font-bold text-2xl sm:text-4xl md:text-5xl shadow-2xl hover:shadow-3xl transition-all duration-200 flex flex-col items-center gap-3 sm:gap-5 border-4 border-destructive/20 active:scale-98 min-h-[140px] sm:min-h-[200px]"
          >
          ...
          <button
            onClick={onReport}
            className="w-full bg-secondary hover:bg-secondary/90 text-secondary-foreground rounded-2xl sm:rounded-3xl p-6 sm:p-12 font-bold text-2xl sm:text-4xl md:text-5xl shadow-2xl hover:shadow-3xl transition-all duration-200 flex flex-col items-center gap-3 sm:gap-5 border-4 border-secondary/20 active:scale-98 min-h-[140px] sm:min-h-[200px]"
          >
          ...
          <button
            onClick={onNews}
            className="w-full bg-accent hover:bg-accent/90 text-accent-foreground rounded-2xl sm:rounded-3xl p-6 sm:p-12 font-bold text-2xl sm:text-4xl md:text-5xl shadow-2xl hover:shadow-3xl transition-all duration-200 flex flex-col items-center gap-3 sm:gap-5 border-4 border-accent/20 active:scale-98 min-h-[140px] sm:min-h-[200px]"
          >
            <span className="text-6xl sm:text-8xl">📢</span>
        ...
        {/* Info Card */}
        <div className="w-full max-w-3xl bg-card rounded-2xl sm:rounded-3xl p-5 sm:p-10 border-4 border-border shadow-xl">
          <p className="text-xl sm:text-3xl font-bold text-center text-foreground mb-2 sm:mb-4">ℹ️ ต้องการความช่วยเหลือ?</p>
          <p className="text-lg sm:text-2xl text-center text-foreground leading-relaxed">
